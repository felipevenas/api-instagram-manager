...
from app.domain.customer.schemas import ResponseCustomer, ResponseCustomerSimple, CreateCustomer, UpdateCustomer
...
@customer_router.get("/", response_model=list[ResponseCustomerSimple], summary=["Buscar todos os clientes do sistema"])
def get_all(service: CustomerService = Depends(get_customer_service)):
    return service.get_all()

@customer_router.get("/{customer_id}", response_model=ResponseCustomerSimple, summary=["Buscar um cliente pelo ID"])
def get_by_id(customer_id: int, service: CustomerService = Depends(get_customer_service)):
    return service.get_by_id(customer_id)

@customer_router.put("/{customer_id}", response_model=UpdateCustomer, summary=["Editar um cliente pelo ID"])
