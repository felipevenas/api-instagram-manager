...
from app.domain.post.schemas import ReadPost, CreatePost, UpdatePost
...
@post_router.post("/register", response_model=ReadPost, summary=["Criar um novo post"])
def create(data: CreatePost, service: PostService = Depends(get_post_service)):
	return service.create(data)

@post_router.get("/", response_model=list[ReadPost], summary=["Listar todos os posts"])
def get_all(service: PostService = Depends(get_post_service)):
	return service.get_all()

@post_router.get("/{post_id}", response_model=ReadPost, summary=["Buscar um post pelo ID"])
def get_by_id(post_id: int, service: PostService = Depends(get_post_service)):
	return service.get_by_id(post_id)

@post_router.put("/{post_id}", response_model=ReadPost, summary=["Atualizar um post pelo ID"])
def update(post_id: int, data: UpdatePost, service: PostService = Depends(get_post_service)):
	return service.update(post_id, data)

@post_router.delete("/{post_id}", response_model=ReadPost, summary=["Deletar um post pelo ID"])
